...
void executeCheck(DiscordCoreAPI::BaseFunctionArguments argsNew, DiscordCoreAPI::DiscordGuildMember* discordFromGuildMember,
	...
	if (*betAmount > fromUserCurrency) {
		...
		auto newEvent02 = DiscordCoreAPI::InputEvents::respondToInputEventAsync(dataPackage).get();
	...
	if (finalFromUserRoll > finalToUserRoll) {
		...
		newEvent = DiscordCoreAPI::InputEvents::respondToInputEventAsync(dataPackageNew).get();
	}
	dataPackageNew.setResponseType(DiscordCoreAPI::InputEventResponseType::Edit_Interaction_Response);
	dataPackageNew.addMessageEmbed(messageEmbeds[0]);
		...
		void execute(BaseFunctionArguments& argsNew) {
			try {
				...
				if (buttonInteractionData.at(0).buttonId == "empty") {
					...
					executeCheck(argsNew, &discordFromGuildMember, &discordToGuildMember, &discordGuild, newEvent02, &betAmount, dataPackageNew, &msgEmbedString, &fromUserID,
						&toUserID);
