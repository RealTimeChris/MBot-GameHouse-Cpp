...
void executeAsyncCrossResponse(std::unique_ptr<DiscordCoreAPI::BaseFunctionArguments> args, DiscordCoreAPI::DiscordGuildMember* discordGuildMember, uint32_t* betAmount, DiscordCoreAPI::GuildMember* guildMember, DiscordCoreAPI::DiscordGuild* discordGuild, DiscordCoreAPI::RespondToInputEventData* buttonInteraction
	,std::vector<DiscordCoreAPI::Card>* userHand, std::vector<uint32_t>* dealerAceIndices, std::string* userID, std::vector<DiscordCoreAPI::Card>* dealerHand, DiscordCoreAPI::EmbedData finalEmbed) {
...
void executeAsyncCheckResponse(std::unique_ptr<DiscordCoreAPI::BaseFunctionArguments> args, DiscordCoreAPI::DiscordGuildMember* discordGuildMember, uint32_t* betAmount, DiscordCoreAPI::GuildMember* guildMember, DiscordCoreAPI::DiscordGuild* discordGuild, DiscordCoreAPI::InputEventData* newEvent, DiscordCoreAPI::RespondToInputEventData* buttonInteraction,
	uint32_t* newCardCount, std::vector<DiscordCoreAPI::Card>* userHand, std::vector<uint32_t>* userAceIndices, std::vector<uint32_t>* dealerAceIndices, std::string* userID, std::vector<DiscordCoreAPI::Card>* dealerHand, DiscordCoreAPI::EmbedData finalEmbed, DiscordCoreAPI::ActionRowData component) {
	...
	if (newUserHandScore > 21) {
		...
		std::string inPlayFooterString = "------\n------";
		DiscordCoreAPI::EmbedData msgEmbed; DiscordCoreAPI::InputEventData newData{};
		msgEmbed.setAuthor(currentUser.userName, currentUser.avatar);
		msgEmbed.setTimeStamp(DiscordCoreAPI::getTimeAndDate());
		...
		std::unique_ptr<std::vector<DiscordCoreAPI::ButtonResponseData>> buttonInteractionData =  std::make_unique<std::vector<DiscordCoreAPI::ButtonResponseData>>(button->collectButtonData(false, 120000, 1, newEvent->getRequesterId()).get());
		...
		if (buttonInteractionData->at(0).buttonId == "check") {
			executeAsyncCheckResponse(std::move(args), discordGuildMember, betAmount, guildMember, discordGuild, newEvent, &inputData, newCardCount, userHand, userAceIndices, dealerAceIndices, userID, dealerHand, finalEmbed, component); 
		}
		else if (buttonInteractionData->at(0).buttonId == "cross") {
			executeAsyncCrossResponse(std::move(args), discordGuildMember, betAmount, guildMember, discordGuild, &inputData, userHand, dealerAceIndices, userID, dealerHand, finalEmbed);
		};
	};
	return;
};

void executeAsyncDoubleResponse(std::unique_ptr<DiscordCoreAPI::BaseFunctionArguments> args, DiscordCoreAPI::DiscordGuildMember* discordGuildMember, uint32_t* betAmount, DiscordCoreAPI::GuildMember* guildMember, DiscordCoreAPI::DiscordGuild* discordGuild, DiscordCoreAPI::InputEventData* newEvent,  DiscordCoreAPI::RespondToInputEventData* buttonInteraction,
	uint32_t* newCardCount, std::vector<DiscordCoreAPI::Card>* userHand, std::vector<uint32_t>* userAceIndices, std::vector<uint32_t>* dealerAceIndices, std::string* userID, std::vector<DiscordCoreAPI::Card>* dealerHand, DiscordCoreAPI::EmbedData finalEmbed, DiscordCoreAPI::ActionRowData component) {
	...
	public:
		...
		virtual void execute( std::unique_ptr<BaseFunctionArguments> args) {
			try {
				...
				if (buttonIntData.at(0).buttonId== "check") {
					executeAsyncCheckResponse(std::move(args), &discordGuildMember, &betAmount, &guildMember, &discordGuild, event001.get(), &buttonInteraction, &newCardCount, &userHand, &userAceIndices, &dealerAceIndices, &userID, &dealerHand, finalMessageEmbed, components.at(0));
				}
				else if (buttonIntData.at(0).buttonId == "cross") {
					executeAsyncCrossResponse(std::move(args), &discordGuildMember, &betAmount, &guildMember, &discordGuild, &buttonInteraction, &userHand, &dealerAceIndices, &userID, &dealerHand, finalMessageEmbed);
				}
				else if (buttonIntData.at(0).buttonId == "double") {
					executeAsyncDoubleResponse(std::move(args), &discordGuildMember, &betAmount, &guildMember, &discordGuild, event001.get(), &buttonInteraction, &newCardCount, &userHand, &userAceIndices, &dealerAceIndices, &userID, &dealerHand, finalMessageEmbed, components.at(0));
				};
				
				return;
			}
				catch (...) {
					reportException("Blackjack::executeAsync()");
				}
			};
